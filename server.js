  ...
  try {
    ...
    campaigns.forEach(c => {
      ...
      campaignsGrouped[c.brand_id].push({
        ...
        shipmentStatus: c.shipment_status||'',
      });
    });
...
app.put('/api/campaigns/:id', requireAuth, requireAdmin, async (req, res) => {
  try {
    const { name, period, startDate, endDate, samples, shipmentStatus } = req.body;
    await sql`UPDATE campaigns SET name=${name}, period=${period||''}, start_date=${startDate||null}, end_date=${endDate||null}, samples=${samples||0}, shipment_status=${shipmentStatus||''} WHERE id=${req.params.id}`;
    res.json({ ok: true });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
// ── Contract Templates ───────────────────────────
app.get('/api/contract-templates/:brandId', requireAuth, async (req, res) => {
  try {
    const rows = await sql`SELECT * FROM contract_templates WHERE brand_id=${req.params.brandId} ORDER BY created_at ASC`;
...
app.post('/api/contract-templates', requireAuth, requireAdmin, async (req, res) => {
  try {
    const { id, brandId, name, fields, bodyHtml } = req.body;
    const fieldsJson = JSON.stringify(fields||[]);
    await sql`INSERT INTO contract_templates (id, brand_id, name, fields, body_html) VALUES (${id}, ${brandId}, ${name}, ${fieldsJson}::jsonb, ${bodyHtml||''})`;
    res.json({ ok: true });
  } catch (err) { res.status(500).json({ error: err.message }); }
});

app.put('/api/contract-templates/:id', requireAuth, requireAdmin, async (req, res) => {
  try {
    const { name, fields, bodyHtml } = req.body;
    const fieldsJson = JSON.stringify(fields||[]);
    await sql`UPDATE contract_templates SET name=${name}, fields=${fieldsJson}::jsonb, body_html=${bodyHtml||''} WHERE id=${req.params.id}`;
    res.json({ ok: true });
  } catch (err) { res.status(500).json({ error: err.message }); }
});

app.delete('/api/contract-templates/:id', requireAuth, requireAdmin, async (req, res) => {
  try {
    await sql`DELETE FROM contract_templates WHERE id=${req.params.id}`;
    res.json({ ok: true });
  } catch (err) { res.status(500).json({ error: err.message }); }
});

// ── Contracts (generated instances) ─────────────
app.get('/api/contracts/:brandId', requireAuth, async (req, res) => {
  try {
    const rows = await sql`
      SELECT c.*, u.email as created_by_email
      FROM contracts c
      LEFT JOIN users u ON u.id = c.created_by
      WHERE c.brand_id=${req.params.brandId}
      ORDER BY c.created_at DESC
      LIMIT 50
    `;
    res.json({ ok: true, contracts: rows.map(r => ({
      id: r.id, templateId: r.template_id, brandId: r.brand_id,
      title: r.title,
      values: r.values || {},
      bodyHtml: r.body_html || '',
...
app.post('/api/contracts', requireAuth, async (req, res) => {
  try {
    const { templateId, brandId, title, values, bodyHtml } = req.body;
    const valuesJson = JSON.stringify(values||{});
    const rows = await sql`
      INSERT INTO contracts (template_id, brand_id, title, values, body_html, created_by)
      VALUES (${templateId||null}, ${brandId}, ${title}, ${valuesJson}::jsonb, ${bodyHtml||''}, ${req.user.id})
      RETURNING id
    `;
    res.json({ ok: true, id: rows[0].id });
  } catch (err) { res.status(500).json({ error: err.message }); }
...
app.delete('/api/contracts/:id', requireAuth, requireAdmin, async (req, res) => {
  try {
    await sql`DELETE FROM contracts WHERE id=${req.params.id}`;
    res.json({ ok: true });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
